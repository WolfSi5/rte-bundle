...
const createComboboxStore = state => plateCore.createStore(`combobox-${state.id}`)(state);

const comboboxStore = plateCore.createStore('combobox')({
  activeId: null,
  byId: {},
  floatingOptions: {},
  highlightedIndex: 0,
...
const getTextFromTrigger = (editor, {
  ...
  while (true) {
    ...
    if (!start) break;
    start = plateCore.getPointBefore(editor, start);
    const charRange = start && plateCore.getRange(editor, start, end);
    const charText = plateCore.getEditorString(editor, charRange);
  ...
  const range = start && plateCore.getRange(editor, start, at);
...
const onKeyDownCombobox = editor => event => {
  ...
  if (plateCore.Hotkeys.isTab(editor, event) || isHotkey('enter', event)) {
    event.preventDefault();
