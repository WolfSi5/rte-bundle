...
const deleteRow = editor => {
  if (plateCore.someNode(editor, {
    ...
    const currentTableItem = plateCore.getAboveNode(editor, {
      match: {
        type: plateCore.getPluginType(editor, ELEMENT_TABLE)
      }
    });
    const currentRowItem = plateCore.getAboveNode(editor, {
      match: {
        type: plateCore.getPluginType(editor, ELEMENT_TR)
      }
    });
...
const deleteTable = editor => {
  if (plateCore.someNode(editor, {
    ...
    const tableItem = plateCore.getAboveNode(editor, {
      match: {
        type: plateCore.getPluginType(editor, ELEMENT_TABLE)
      }
    });
...
const insertTable = (editor, {
  ...
  plateCore.withoutNormalizing(editor, () => {
    if (!plateCore.someNode(editor, {
      ...
      if (editor.selection) {
        const tableEntry = plateCore.getBlockAbove(editor, {
          match: {
            type: plateCore.getPluginType(editor, ELEMENT_TABLE)
          }
        });
        ...
        plateCore.selectEditor(editor, {
          at: plateCore.getStartPoint(editor, tableEntry[1])
        });
      }
    }
  });
...
const getCellTypes = editor => plateCore.getPluginTypes(editor, [ELEMENT_TD, ELEMENT_TH]);
...
const insertTableColumn = (editor, {
  ...
  const cellEntry = fromCell ? plateCore.findNode(editor, {
    at: fromCell,
    match: {
      type: getCellTypes(editor)
    }
  }) : plateCore.getBlockAbove(editor, {
    match: {
      type: getCellTypes(editor)
    }
  });
  ...
  if (!tableEntry) return;
  const [tableNode, tablePath] = tableEntry;
  const nextCellPath = slate.Path.next(cellPath);
  const nextColIndex = cellPath[cellPath.length - 1] + 1;
  const currentRowIndex = cellPath[cellPath.length - 2];
  const {
    newCellChildren
  } = plateCore.getPluginOptions(editor, ELEMENT_TABLE);
  plateCore.withoutNormalizing(editor, () => {
    // for each row, insert a new cell
    tableNode.children.forEach((row, rowIndex) => {
      const insertCellPath = [...nextCellPath];
      insertCellPath[cellPath.length - 2] = rowIndex;
      const isHeaderRow = header === undefined ? row.children[0].type === plateCore.getPluginType(editor, ELEMENT_TH) : header;
      plateCore.insertElements(editor, getEmptyCellNode(editor, {
        header: isHeaderRow,
        newCellChildren
      }), {
        at: insertCellPath,
...
const insertTableRow = (editor, {
  ...
  const trEntry = fromRow ? plateCore.findNode(editor, {
    at: fromRow,
    match: {
      type: plateCore.getPluginType(editor, ELEMENT_TR)
    }
  ...
  plateCore.withoutNormalizing(editor, () => {
    plateCore.insertElements(editor, getEmptyRowNode(editor, {
      header,
      colCount: trNode.children.length,
      newCellChildren
    }), {
      at: slate.Path.next(trPath)
    });
  });

  if (!disableSelect) {
    const cellEntry = plateCore.getBlockAbove(editor, {
      match: {
        type: getCellTypes(editor)
      }
    });
    ...
    plateCore.select(editor, nextCellPath);
  }
};
...
const getTableGridByRange = (editor, {
  ...
  while (true) {
    ...
    const cell = plateCore.getNode(editor, cellPath);
    if (!cell) break;
...
const getTableGridAbove = (editor, {
  ...
  const edges = plateCore.getEdgeBlocksAbove(editor, {
    match: {
      type: getCellTypes(editor)
    },
    ...options
...
const moveSelectionFromCell = (editor, {
  ...
  if (edge) {
    ...
    if (cellEntries.length > minCell) {
      ...
      if (plateCore.hasNode(editor, anchorPath) && plateCore.hasNode(editor, focusPath)) {
        plateCore.select(editor, {
          anchor: plateCore.getStartPoint(editor, anchorPath),
          focus: plateCore.getStartPoint(editor, focusPath)
        });
      }
  ...
  if (cellEntry) {
    ...
    if (plateCore.hasNode(editor, nextCellPath)) {
      plateCore.select(editor, plateCore.getStartPoint(editor, nextCellPath));
    } else {
      const tablePath = cellPath.slice(0, -2);

      if (reverse) {
        plateCore.withoutNormalizing(editor, () => {
          plateCore.select(editor, plateCore.getStartPoint(editor, tablePath));
          plateCore.moveSelection(editor, {
            reverse: true
          });
        });
      } else {
        plateCore.withoutNormalizing(editor, () => {
          plateCore.select(editor, plateCore.getEndPoint(editor, tablePath));
          plateCore.moveSelection(editor);
        });
      }
    }
...
const overrideSelectionFromCell = (editor, newSelection) => {
  ...
  if (!editor.currentKeyboardEvent || !['up', 'down', 'shift+up', 'shift+right', 'shift+down', 'shift+left'].some(key => {
    ...
    if (valid) hotkey = key;
    return valid;
  }) || !((_editor$selection = editor.selection) !== null && _editor$selection !== void 0 && _editor$selection.focus) || !(newSelection !== null && newSelection !== void 0 && newSelection.focus) || !plateCore.isRangeAcrossBlocks(editor, {
    at: {
  ...
  if (edge && !plateCore.isRangeInSameBlock(editor, {
    at: editor.selection,
    match: {
      type: getCellTypes(editor)
    }
...
const getCellInNextTableRow = (editor, currentRowPath) => {
  try {
    ...
    const nextRow = plateCore.getNodeEntry(editor, slate.Path.next(currentRowPath)); // TODO: Many tables in rich text editors (Google Docs, Word),
    ...
    if (nextCell && nextCellPath) {
      return plateCore.getNodeEntry(editor, nextCellPath);
    }
  } catch (err) {}
};
...
const getCellInPreviousTableRow = (editor, currentRowPath) => {
  try {
    ...
    const previousRow = plateCore.getNodeEntry(editor, slate.Path.previous(currentRowPath));
    ...
    if (previousCell && previousCellPath) {
      return plateCore.getNodeEntry(editor, previousCellPath);
    }
  } catch (err) {}
};
...
const getNextTableCell = (editor, currentCell, currentPath, currentRow) => {
  try {
    return plateCore.getNodeEntry(editor, slate.Path.next(currentPath));
  } catch (err) {
...
const getPreviousTableCell = (editor, currentCell, currentPath, currentRow) => {
  try {
    return plateCore.getNodeEntry(editor, slate.Path.previous(currentPath));
  } catch (err) {
    const [, currentRowPath] = currentRow;
...
const getTableAbove = (editor, options) => plateCore.getBlockAbove(editor, {
  match: {
    type: plateCore.getPluginType(editor, ELEMENT_TABLE)
  },
  ...options
...
const getTableColumnIndex = (editor, {
  ...
  if (!path) return 0;
  const [trNode] = (_getParentNode = plateCore.getParentNode(editor, path)) !== null && _getParentNode !== void 0 ? _getParentNode : [];
  if (!trNode) return 0;
  let colIndex = 0;
...
const getTableEntries = (editor, {
  ...
  if (!at) return;
  const cellEntry = plateCore.findNode(editor, {
    at,
    match: {
      type: getCellTypes(editor)
    }
  ...
  if (!cellEntry) return;
  const [, cellPath] = cellEntry;
  const rowEntry = plateCore.getAboveNode(editor, {
    at: cellPath,
    match: {
      type: plateCore.getPluginType(editor, ELEMENT_TR)
    }
...
const onKeyDownTable = (editor, {
  ...
  if (isTab || isUntab) {
    ...
    if (isUntab) {
      ...
      if (previousCell) {
        ...
        plateCore.select(editor, previousCellPath);
      }
    } else if (isTab) {
      // move right with tab
  ...
  if (isHotkey('mod+a', e)) {
    const res = plateCore.getAboveNode(editor, {
      match: {
        type
      }
    });
    ...
    plateCore.select(editor, tablePath);
    e.preventDefault();
    e.stopPropagation();
  }
};
...
const preventDeleteTableCell = (editor, {
  ...
  const getPoint = !reverse ? plateCore.getStartPoint : plateCore.getEndPoint;
  const getNextPoint = !reverse ? plateCore.getPointBefore : plateCore.getPointAfter;
  ...
  if (plateCore.isCollapsed(selection)) {
    ...
    if (cellEntry) {
      ...
      const nextCellEntry = plateCore.getBlockAbove(editor, {
        match: {
          type: getCellTypes(editor)
        },
        at: nextPoint
...
const withDeleteTable = editor => {
  ...
  editor.deleteFragment = () => {
    plateCore.isRangeInSameBlock(editor, {
      match: n => n.type === plateCore.getPluginType(editor, ELEMENT_TABLE)
    });

    if (plateCore.isRangeInSameBlock(editor, {
      match: n => n.type === plateCore.getPluginType(editor, ELEMENT_TABLE)
    })) {
      const cellEntries = getTableGridAbove(editor, {
        format: 'cell'
      ...
      if (cellEntries.length > 1) {
        plateCore.withoutNormalizing(editor, () => {
          ...
          plateCore.select(editor, {
            anchor: plateCore.getStartPoint(editor, cellEntries[0][1]),
            focus: plateCore.getEndPoint(editor, cellEntries[cellEntries.length - 1][1])
          });
        });
        return;
      }
...
const withGetFragmentTable = editor => {
  ...
  editor.getFragment = () => {
    ...
    fragment.forEach(node => {
      if (node.type === plateCore.getPluginType(editor, ELEMENT_TABLE)) {
...
const withInsertFragmentTable = (editor, {
  ...
  const myEditor = plateCore.getTEditor(editor);

  myEditor.insertFragment = fragment => {
    const insertedTable = fragment.find(n => n.type === plateCore.getPluginType(editor, ELEMENT_TABLE));
          ...
          plateCore.withoutNormalizing(editor, () => {
            ...
            insertedRows.forEach(row => {
              ...
              if (!initRow) {
                ...
                if (!plateCore.hasNode(editor, cellPath)) {
            ...
            if (lastCellPath) {
              plateCore.select(editor, {
                anchor: plateCore.getStartPoint(editor, startCellPath),
                focus: plateCore.getEndPoint(editor, lastCellPath)
              });
            }
          });
          return;
...
const withInsertTextTable = (editor, plugin) => {
  ...
  editor.insertText = text => {
    if (plateCore.isExpanded(editor.selection)) {
      var _editor$selection;
...
const withNormalizeTable = editor => {
  ...
  myEditor.normalizeNode = ([node, path]) => {
    if (plateCore.isElement(node)) {
      ...
      if (node.type === plateCore.getPluginType(editor, ELEMENT_TR)) {
        const parentEntry = plateCore.getParentNode(editor, path);

        if ((parentEntry === null || parentEntry === void 0 ? void 0 : parentEntry[0].type) !== plateCore.getPluginType(editor, ELEMENT_TABLE)) {
          plateCore.unwrapNodes(editor, {
            at: path
          });
          return;
      ...
      if (getCellTypes(editor).includes(node.type)) {
        ...
        if ((parentEntry === null || parentEntry === void 0 ? void 0 : parentEntry[0].type) !== plateCore.getPluginType(editor, ELEMENT_TR)) {
          plateCore.unwrapNodes(editor, {
            at: path
          });
          return;
        ...
        if (plateCore.isText(children[0])) {
          plateCore.wrapNodeChildren(editor, {
            type: plateCore.getPluginType(editor, plateCore.ELEMENT_DEFAULT),
            children: [{
              text: ''
            }]
...
const withSelectionTable = editor => {
  ...
  editor.apply = op => {
    if (op.type === 'set_selection' && op.newProperties) {
      ...
      if (slate.Range.isRange(newSelection) && plateCore.isRangeAcrossBlocks(editor, {
        at: newSelection,
        match: n => n.type === plateCore.getPluginType(editor, ELEMENT_TABLE)
      })) {
        const anchorEntry = plateCore.getBlockAbove(editor, {
          at: newSelection.anchor,
          match: n => n.type === plateCore.getPluginType(editor, ELEMENT_TABLE)
        });

        if (!anchorEntry) {
          const focusEntry = plateCore.getBlockAbove(editor, {
          ...
          if (focusEntry) {
            ...
            if (isBackward) {
              const startPoint = plateCore.getStartPoint(editor, focusPath);
              const pointBefore = plateCore.getPointBefore(editor, startPoint);
              op.newProperties.focus = pointBefore !== null && pointBefore !== void 0 ? pointBefore : startPoint;
            } else {
              op.newProperties.focus = plateCore.getEndPoint(editor, focusPath);
            }
          }
        } else {
          ...
          if (isBackward) {
            op.newProperties.focus = plateCore.getStartPoint(editor, anchorPath);
          } else {
            const pointBefore = plateCore.getPointBefore(editor, anchorPath); // if the table is the first block

            if (pointBefore) {
              op.newProperties.focus = plateCore.getEndPoint(editor, anchorPath);
            }
          }
        }
      }
