...
const getSiblingIndentList = (editor, [node, path], {
  ...
  while (true) {
    ...
    if (!plateCore.isDefined(nextIndent)) return;
...
const getPreviousIndentList = (editor, entry, options) => {
  return getSiblingIndentList(editor, entry, {
    getPreviousEntry: ([, currPath]) => {
      const prevPath = plateCore.getPreviousPath(currPath);
      if (!prevPath) return;
...
const toggleIndentList = (editor, options) => {
  ...
  const {
    getSiblingIndentListOptions
  } = plateCore.getPluginOptions(editor, KEY_LIST_STYLE_TYPE);
  ...
  if (plateCore.isExpanded(editor.selection)) {
    const _entries = plateCore.getNodeEntries(editor, {
      block: true
    });
      ...
      plateCore.withoutNormalizing(editor, () => {
        entries.forEach(entry => {
          ...
          if (indent > 1) {
            ...
            plateCore.unsetNodes(editor, plateIndent.KEY_INDENT, {
              at: path
            });
          } // setIndentListNode(editor, {
          //   listStyleType,
...
const onKeyDownIndentList = (editor, plugin) => e => {
  ...
  if (isHotkey('Enter', e)) {
    if (plateCore.isBlockAboveEmpty(editor) && node.indent) {
      outdentList(editor);
      e.stopPropagation();
      e.preventDefault();
    }
...
const normalizeFirstIndentListStart = (editor, [node, path]) => {
  if (plateCore.isDefined(node[KEY_LIST_START])) {
    plateCore.unsetNodes(editor, KEY_LIST_START, {
      at: path
    });
    return true;
  }
...
const normalizeIndentListStart = (editor, entry, options) => {
  return plateCore.withoutNormalizing(editor, () => {
    ...
    while (normalizeNext) {
      ...
      prevEntry = [plateCore.getNode(editor, currEntry[1]), currEntry[1]];
...
const normalizeIndentListNotIndented = (editor, [node, path]) => {
  if (!plateCore.isDefined(node[plateIndent.KEY_INDENT]) && (node[KEY_LIST_STYLE_TYPE] || node[KEY_LIST_START])) {
    plateCore.unsetNodes(editor, [KEY_LIST_STYLE_TYPE, KEY_LIST_START], {
      at: path
    });
    return true;
  }
...
const normalizeIndentList = (editor, {
  ...
  return ([node, path]) => {
    const normalized = plateCore.withoutNormalizing(editor, () => {
    ...
    if (operation.type === 'merge_node' && operation.properties[KEY_LIST_STYLE_TYPE]) {
      ...
      if (node) {
        ...
        if (nextNodeEntryBefore) {
          nextIndentListPathRef = plateCore.createPathRef(editor, nextNodeEntryBefore[1]);
        }
      }
    }
