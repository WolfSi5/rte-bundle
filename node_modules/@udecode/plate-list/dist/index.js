...
const isListNested = (editor, listPath) => {
  ...
  const listParentNode = (_getParentNode = plateCore.getParentNode(editor, listPath)) === null || _getParentNode === void 0 ? void 0 : _getParentNode[0];
  return (listParentNode === null || listParentNode === void 0 ? void 0 : listParentNode.type) === plateCore.getPluginType(editor, ELEMENT_LI);
};

const getUnorderedListType = editor => {
  return plateCore.getPluginType(editor, ELEMENT_UL);
};
const getOrderedListType = editor => {
  return plateCore.getPluginType(editor, ELEMENT_OL);
};
const getListTypes = editor => {
  return [getOrderedListType(editor), getUnorderedListType(editor)];
};
const getListItemType = editor => {
  return plateCore.getPluginType(editor, ELEMENT_LI);
};
const getListItemContentType = editor => {
  return plateCore.getPluginType(editor, ELEMENT_LIC);
};
...
const getHighestEmptyList = (editor, {
  ...
  const list = plateCore.getAboveNode(editor, {
    at: liPath,
    match: {
  ...
  if (!diffListPath || !slate.Path.equals(listPath, diffListPath)) {
    if (listNode.children.length < 2) {
      const liParent = plateCore.getAboveNode(editor, {
        at: listPath,
        match: {
          type: plateCore.getPluginType(editor, ELEMENT_LI)
        }
      });
...
const getListItemEntry = (editor, {
  ...
  const liType = plateCore.getPluginType(editor, ELEMENT_LI);

  let _at;

  if (slate.Range.isRange(at) && !plateCore.isCollapsed(at)) {
    _at = at.focus.path;
  ...
  if (_at) {
    const node = plateCore.getNode(editor, _at);

    if (node) {
      const listItem = plateCore.getAboveNode(editor, {
        at: _at,
        match: {
          type: liType
        }
      ...
      if (listItem) {
        const list = plateCore.getParentNode(editor, listItem[1]);
        return {
          list,
          listItem
        };
...
const getListRoot = (editor, at = editor.selection) => {
  if (!at) return;
  const parentList = plateCore.getAboveNode(editor, {
    at,
    match: {
      type: [plateCore.getPluginType(editor, ELEMENT_UL), plateCore.getPluginType(editor, ELEMENT_OL)]
    }
  });
...
const hasListChild = (editor, node) => node.children.some(n => plateCore.match(n, [], {
  type: getListTypes(editor)
}));
...
const isAcrossListItems = editor => {
  ...
  const isAcrossBlocks = plateCore.isRangeAcrossBlocks(editor);
  if (!isAcrossBlocks) return false;
  return plateCore.someNode(editor, {
    match: {
      type: plateCore.getPluginType(editor, ELEMENT_LI)
    }
  });
...
const isListRoot = (editor, node) => plateCore.isElement(node) && getListTypes(editor).includes(node.type);
...
const moveListItemDown = (editor, {
  ...
  const previousSiblingItem = plateCore.getNodeEntry(editor, previousListItemPath);

  if (previousSiblingItem) {
    const [previousNode, previousPath] = previousSiblingItem;
    const sublist = previousNode.children.find(n => plateCore.match(n, [], {
      type: getListTypes(editor)
    }));
...
const moveListItemsToList = (editor, {
  ...
  plateCore.withoutNormalizing(editor, () => {
    if (fromListItem) {
      const fromListItemSublist = plateCore.findDescendant(editor, {
        at: fromListItem[1],
        match: {
          type: getListTypes(editor)
        }
    ...
    if (toList) {
      if (toListIndex !== null) to = toList[1].concat([toListIndex]);else {
        const lastChildPath = plateCore.getLastChildPath(toList);
        to = slate.Path.next(lastChildPath);
      }
    }

    if (!to) return;
    moved = plateCore.moveChildren(editor, {
      at: fromListPath,
      to,
      fromStartIndex
...
const unwrapList = (editor, {
  ...
  const ancestorListTypeCheck = () => {
    if (plateCore.getAboveNode(editor, {
      match: {
        type: getListTypes(editor),
        at
      }
    ...
    if (!at && editor.selection) {
      const commonNode = plateCore.getCommonNode(editor, editor.selection.anchor.path, editor.selection.focus.path);

      if (plateCore.isElement(commonNode[0]) && getListTypes(editor).includes(commonNode[0].type)) {
        return true;
      }
    }
  ...
  plateCore.withoutNormalizing(editor, () => {
    do {
      const licEntry = plateCore.getBlockAbove(editor, {
        at,
        match: {
          type: plateCore.getPluginType(editor, ELEMENT_LIC)
        }
      });
...
const moveListItemUp = (editor, {
  ...
  const move = () => {
    ...
    if (!liParent) {
      ...
      const condB = !plateCore.isLastChild(list, liPath);

      if (condA || condB) {
        // Insert a new list next to `list`
        plateCore.insertElements(editor, {
          type: listNode.type,
          children: []
        }, {
          at: toListPath
      ...
      if (condA) {
        const toListNode = plateCore.getNode(editor, toListPath);
    ...
    if (!plateCore.isLastChild(list, liPath)) {
...
const moveListItems = (editor, {
  ...
  lics.forEach(lic => {
    ...
    if (!isAncestor) {
      ...
      highestLicPathRefs.push(plateCore.createPathRef(editor, licPath));
    }
  });
  ...
  return plateCore.withoutNormalizing(editor, () => {
    ...
    licPathRefsToMove.forEach(licPathRef => {
      ...
      if (!licPath) return;
      const listItem = plateCore.getParentNode(editor, licPath);
      if (!listItem) return;
      const parentList = plateCore.getParentNode(editor, listItem[1]);
...
const insertListItem = editor => {
  const liType = plateCore.getPluginType(editor, ELEMENT_LI);
  const licType = plateCore.getPluginType(editor, ELEMENT_LIC);

  if (!editor.selection) {
    return false;
  }

  const licEntry = plateCore.getAboveNode(editor, {
    match: {
      type: licType
    }
  });
  ...
  const listItemEntry = plateCore.getParentNode(editor, paragraphPath);
  ...
  plateCore.withoutNormalizing(editor, () => {
    ...
    const isStart = plateCore.isStartPoint(editor, editor.selection.focus, paragraphPath);
    const isEnd = plateCore.isBlockTextEmptyAfterSelection(editor);
    ...
    if (!isEnd) {
      plateCore.withoutNormalizing(editor, () => {
        ...
        plateCore.select(editor, nextListItemPath);
       ...
       */
      const marks = plateCore.getMarks(editor) || {};
      plateCore.insertElements(editor, {
        type: liType,
        children: [{
          type: licType,
...
const moveListItemSublistItemsToListItemSublist = (editor, {
  ...
  plateCore.withoutNormalizing(editor, () => {
    ...
    const toListItemSublist = plateCore.findDescendant(editor, {
      at: toListItemPath,
      match: {
        type: getListTypes(editor)
      }
    ...
    if (!toListItemSublist) {
      const fromList = plateCore.getParentNode(editor, fromListItemPath);
      if (!fromList) return;
      ...
      to = slate.Path.next(plateCore.getLastChildPath(toListItemSublist));
    }
...
const moveListSiblingsAfterCursor = (editor, {
  ...
  const listNode = plateCore.getNode(editor, at);
  const listEntry = [listNode, at];

  if (!plateCore.match(listNode, [], {
    type: getListTypes(editor)
  ...
  return plateCore.moveChildren(editor, {
    at: listEntry,
    to,
...
const removeListItem = (editor, {
  ...
  if (plateCore.isExpanded(editor.selection) || !hasListChild(editor, liNode)) {
    return false;
  }

  const previousLiPath = plateCore.getPreviousPath(liPath);
  let success = false;
  plateCore.withoutNormalizing(editor, () => {
    /**
     ...
     */
    if (previousLiPath) {
      const previousLi = plateCore.getNodeEntry(editor, previousLiPath); // 1
      ...
      const tempLi = plateCore.getNodeEntry(editor, tempLiPath);
      const tempLiPathRef = plateCore.createPathRef(editor, tempLi[1]); // 2
...
const toggleList = (editor, {
  ...
  if (plateCore.isCollapsed(editor.selection) || !plateCore.isRangeAcrossBlocks(editor)) {
    ...
    if (res) {
      ...
      if (list[0].type !== type) {
        plateCore.setElements(editor, {
          ...
          at: editor.selection,
          match: n => plateCore.isElement(n) && getListTypes(editor).includes(n.type),
          mode: 'lowest'
        });
      } else {
        unwrapList(editor);
      ...
      plateCore.wrapNodes(editor, list);
    ...
    const commonEntry = plateCore.getCommonNode(editor, startPoint.path, endPoint.path);

    if (getListTypes(editor).includes(commonEntry[0].type) || commonEntry[0].type === plateCore.getPluginType(editor, ELEMENT_LI)) {
      if (commonEntry[0].type !== type) {
        const startList = plateCore.findNode(editor, {
        ...
        const endList = plateCore.findNode(editor, {
        ...
        plateCore.setElements(editor, {
          ...
          at: editor.selection,
          match: (n, path) => plateCore.isElement(n) && getListTypes(editor).includes(n.type) && path.length >= rangeLength,
          mode: 'all'
        });
      } else {
        unwrapList(editor);
      ...
      nodes.forEach(n => {
        if (getListTypes(editor).includes(n[0].type)) {
          plateCore.setElements(editor, {
            ...
            at: n[1],
            match: _n => plateCore.isElement(_n) && getListTypes(editor).includes(_n.type),
            mode: 'all'
          });
        } else {
...
const onKeyDownList = (editor, {
  ...
  const isTab = plateCore.Hotkeys.isTab(editor, e);
  const isUntab = plateCore.Hotkeys.isUntab(editor, e);
  ...
  if (editor.selection && (isTab || isUntab)) {
    ...
    if (!plateCore.isCollapsed(editor.selection)) {
      ...
      anchor.offset = 0;
      const unHungRange = plateCore.unhangRange(editor, {
        anchor,
        focus
      });
      ...
      if (unHungRange) {
        ...
        plateCore.select(editor, unHungRange);
      }
    } // check if we're in a list context.


    const listSelected = plateCore.someNode(editor, {
      match: {
        type: plateCore.getPluginType(editor, ELEMENT_LI)
      }
    });
...
const deleteBackwardList = (editor, unit) => {
  ...
  if (res) {
    ...
    if (plateCore.isSelectionAtBlockStart(editor, {
      match: node => node.type === plateCore.getPluginType(editor, ELEMENT_LI)
    })) {
      plateCore.withoutNormalizing(editor, () => {
        ...
        if (plateCore.isFirstChild(listItem[1]) && !isListNested(editor, list[1])) {
          plateResetNode.onKeyDownResetNode(editor, plateCore.mockPlugin({
            options: {
              rules: [{
                types: [plateCore.getPluginType(editor, ELEMENT_LI)],
                defaultType: plateCore.getPluginType(editor, plateCore.ELEMENT_DEFAULT),
                hotkey: 'backspace',
                predicate: () => plateCore.isSelectionAtBlockStart(editor),
...
const selectionIsNotInAListHandler = editor => {
  const pointAfterSelection = plateCore.getPointAfter(editor, editor.selection.focus.path);
  ...
  if (pointAfterSelection) {
    ...
    if (nextSiblingListRes) {
      ...
      const parentBlockEntity = plateCore.getBlockAbove(editor, {
        at: editor.selection.anchor
      });

      if (!plateCore.getEditorString(editor, parentBlockEntity[1])) {
...
const selectionIsInAListHandler = (editor, res) => {
  ...
  if (!hasListChild(editor, listItem[0])) {
    ...
    const _nodes = plateCore.getNodeEntries(editor, {
      ...
      match: (node, path) => {
        ...
        const isSiblingOfNodeLi = ((_getNode = plateCore.getNode(editor, slate.Path.next(path))) === null || _getNode === void 0 ? void 0 : _getNode.type) === liType;
    ...
    if (!liWithSiblings) {
      ...
      const pointAfterListItem = plateCore.getPointAfter(editor, listItem[1]);
    ...
    const siblingListItem = plateCore.getNodeEntry(editor, slate.Path.next(liWithSiblings));
    const siblingList = plateCore.getParentNode(editor, siblingListItem[1]);
  ...
  const nestedList = plateCore.getNodeEntry(editor, slate.Path.next([...listItem[1], 0]));
  const nestedListItem = plateCore.getChildren(nestedList)[0];
...
const deleteForwardList = editor => {
  ...
  if (!plateCore.isSelectionAtBlockEnd(editor)) {
    return skipDefaultDelete;
  }
...
const deleteFragmentList = editor => {
  ...
  plateCore.withoutNormalizing(editor, () => {
    ...
    const end = plateCore.getEndPoint(editor, editor.selection);
    const liEnd = plateCore.getAboveNode(editor, {
      at: end,
      match: {
        type: plateCore.getPluginType(editor, ELEMENT_LI)
      }
    ...
    const liEndPathRef = liEndCanBeDeleted ? plateCore.createPathRef(editor, liEnd[1]) : undefined;
    /**
     * Delete fragment and move end block children to start block
     */

    plateCore.deleteMerge(editor);
    const start = plateCore.getStartPoint(editor, editor.selection);
    const liStart = plateCore.getAboveNode(editor, {
      at: start,
      match: {
        type: plateCore.getPluginType(editor, ELEMENT_LI)
      }
    ...
    if (liEndPathRef) {
      ...
      const listStart = liStart && plateCore.getParentNode(editor, liStart[1]);
...
const insertBreakList = editor => {
  ...
  if (res) {
    ...
    if (plateCore.isBlockAboveEmpty(editor)) {
  ...
  const didReset = plateResetNode.onKeyDownResetNode(editor, plateCore.mockPlugin({
    options: {
      rules: [{
        types: [plateCore.getPluginType(editor, ELEMENT_LI)],
        defaultType: plateCore.getPluginType(editor, plateCore.ELEMENT_DEFAULT),
        predicate: () => !moved && plateCore.isBlockAboveEmpty(editor),
...
const insertFragmentList = editor => {
  ...
  const listItemPlugin = plateCore.getPlugin(editor, ELEMENT_LI);
  ...
  const getFirstAncestorOfType = (root, entry, {
    ...
    while (plateCore.getNode(root, ancestor).type !== type) {
      ancestor = slate.Path.parent(ancestor);
    }

    return [plateCore.getNode(root, ancestor), ancestor];
  };
  ...
  const trimList = listRoot => {
    ...
    const _texts = plateCore.getNodeTexts(listRoot);

    const textEntries = Array.from(_texts);
    const commonAncestorEntry = textEntries.reduce((commonAncestor, textEntry) => slate.Path.isAncestor(commonAncestor[1], textEntry[1]) ? commonAncestor : plateCore.getCommonNode(listRoot, textEntry[1], commonAncestor[1]), // any list item would do, we grab the first one
  ...
  const isSingleLic = fragment => {
    ...
    return isFragmentOnlyListRoot && [...plateCore.getNodes({
      children: fragment
    })].filter(entry => plateCore.isElement(entry[0])).filter(([node]) => node.type === listItemContentType).length === 1;
  };

  const getTextAndListItemNodes = (fragment, liEntry, licEntry) => {
    const [, liPath] = liEntry;
    const [licNode, licPath] = licEntry;
    const isEmptyNode = !plateCore.getNodeString(licNode);
    ...
    if (isListRoot(editor, fragment[0])) {
      if (isSingleLic(fragment)) {
        ...
        const li = plateCore.getNode(editor, liPath);
        const [, ...currentSublists] = li.children;
        const [newLic, ...newSublists] = first.children;
        plateCore.insertElements(editor, newLic, {
        ...
        if (newSublists !== null && newSublists !== void 0 && newSublists.length) {
          if (currentSublists !== null && currentSublists !== void 0 && currentSublists.length) {
            ...
            plateCore.insertElements(editor, newSublists[0].children, {
              at: path,
              select: true
            });
          } else {
            plateCore.insertElements(editor, newSublists, {
              at: slate.Path.next(licPath),
              select: true
            });
          }
  ...
  return fragment => {
    let liEntry = plateCore.findNode(editor, {
      match: {
        type: listItemType
      },
      mode: 'lowest'
    ...
    plateCore.insertFragment(editor, [{
      text: ''
    }]); // refetch to find the currently selected LI after the deletion above is performed

    liEntry = plateCore.findNode(editor, {
      match: {
        type: listItemType
      },
      mode: 'lowest'
    });
    const licEntry = plateCore.findNode(editor, {
      match: {
    ...
    plateCore.insertFragment(editor, [textNode]); // insert text if needed

    const [, liPath] = liEntry;
    return plateCore.insertElements(editor, listItemNodes, {
...
const getDeepInlineChildren = (editor, {
  ...
  for (const child of children) {
    if (plateCore.isBlock(editor, child[0])) {
      inlineChildren.push(...getDeepInlineChildren(editor, {
        children: plateCore.getChildren(child)
      }));
    } else {
...
const normalizeListItem = (editor, {
  ...
  let changed = false;
  const allValidLiChildrenTypes = [plateCore.getPluginType(editor, ELEMENT_UL), plateCore.getPluginType(editor, ELEMENT_OL), plateCore.getPluginType(editor, ELEMENT_LIC), ...validLiChildrenTypes];
  const [, liPath] = listItem;
  const liChildren = plateCore.getChildren(listItem); // Get invalid (type) li children path refs to be moved

  const invalidLiChildrenPathRefs = liChildren.filter(([child]) => !allValidLiChildrenTypes.includes(child.type)).map(([, childPath]) => plateCore.createPathRef(editor, childPath));
  ...
  if (!firstLiChild || !plateCore.isBlock(editor, firstLiChildNode)) {
    plateCore.insertEmptyElement(editor, plateCore.getPluginType(editor, ELEMENT_LIC), {
      at: liPath.concat([0])
    });
    return true;
  ...
  if (plateCore.isBlock(editor, firstLiChildNode) && !plateCore.match(firstLiChildNode, [], {
    type: plateCore.getPluginType(editor, ELEMENT_LIC)
  })) {
    if (plateCore.match(firstLiChildNode, [], {
      type: getListTypes(editor)
    })) {
      // the listItem has no lic so we move the children up a level
      const parent = plateCore.getParentNode(editor, listItem[1]);
      const sublist = firstLiChild;
      const children = plateCore.getChildren(firstLiChild).reverse();
      children.forEach(c => {
        moveListItemUp(editor, {
          list: sublist,
          listItem: c
  ...
  const licChildren = plateCore.getChildren(firstLiChild);
  ...
  if (licChildren.length) {
    ...
    for (const licChild of licChildren) {
      if (!plateCore.isBlock(editor, licChild[0])) {
        break;
      }

      blockPathRefs.push(plateCore.createPathRef(editor, licChild[1]));
      inlineChildren.push(...getDeepInlineChildren(editor, {
        children: plateCore.getChildren(licChild)
      }));
    }
    ...
    blockPathRefs.forEach(pathRef => {
      ...
      path && plateCore.removeNodes(editor, {
        at: path
      });
    });
  ...
  invalidLiChildrenPathRefs.reverse().forEach(ref => {
    ...
    path && plateCore.moveNodes(editor, {
      at: path,
...
const normalizeNestedList = (editor, {
  ...
  const parentNode = plateCore.getParentNode(editor, path);
  const hasParentList = parentNode && plateCore.match(parentNode[0], [], {
    type: getListTypes(editor)
  });
...
const normalizeList = (editor, {
  ...
  return ([node, path]) => {
    ...
    if (isListRoot(editor, node)) {
      const nonLiChild = plateCore.getChildren([node, path]).find(([child]) => child.type !== liType);

      if (nonLiChild) {
        return plateCore.wrapNodes(editor, {
          type: liType,
          children: []
        }, {
          at: nonLiChild[1]
    ...
    if (plateCore.match(node, [], {
      type: getListTypes(editor)
    })) {
      if (!node.children.length || !node.children.find(item => item.type === liType)) {
        return plateCore.removeNodes(editor, {
          at: path
        });
      }

      const nextPath = slate.Path.next(path);
      const nextNode = plateCore.getNode(editor, nextPath); // Has a list afterwards with the same type
      ...
      const prevPath = plateCore.getPreviousPath(path);
      const prevNode = plateCore.getNode(editor, prevPath); // Has a list before with the same type
    ...
    if (node.type === plateCore.getPluginType(editor, ELEMENT_LI)) {
    ...
    if (node.type === licType && licType !== defaultType) {
      ...
      if (((_getParentNode = plateCore.getParentNode(editor, path)) === null || _getParentNode === void 0 ? void 0 : _getParentNode[0].type) !== liType) {
        plateCore.setElements(editor, {
          type: defaultType
        }, {
          at: path
...
const createListPlugin = plateCore.createPluginFactory({
  key: 'list',
  plugins: [{
    key: ELEMENT_UL,
    isElement: true,
    ...
    then: (editor, {
      ...
      inject: {
        pluginsByKey: {
          [plateCore.KEY_DESERIALIZE_HTML]: {
            editor: {
              insertData: {
                preInsert: () => {
                  return plateCore.someNode(editor, {
...
const createTodoListPlugin = plateCore.createPluginFactory({
  key: ELEMENT_TODO_LI,
  isElement: true,
  handlers: {
