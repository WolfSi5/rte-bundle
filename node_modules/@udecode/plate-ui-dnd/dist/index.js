...
const Draggable = props => {
  ...
  }, children, !!dropLine && /*#__PURE__*/React__default["default"].createElement(_StyledDiv6, {
...
const withDraggable = (Component, {
  ...
  return /*#__PURE__*/React.forwardRef((props, ref) => {
    ...
    const filteredOut = React.useMemo(() => path && (Number.isInteger(level) && level !== path.length - 1 || filter && filter(editor, path)), [path, editor]);
...
const getBlocksWithId = (editor, options) => {
  const _nodes = plateCore.getNodeEntries(editor, {
    match: n => plateCore.isBlock(editor, n) && !!n.id,
    ...options
  });
...
const getNodesRange = (editor, nodeEntries) => {
  ...
  return plateCore.getRange(editor, firstBlockPath, lastBlockPath);
};
...
const focusBlockStartById = (editor, id) => {
  ...
  if (!path) return;
  plateCore.select(editor, plateCore.getStartPoint(editor, path));
  plateCore.focusEditor(editor);
};
...
const removeBlocksAndFocus = (editor, options) => {
  plateCore.unhangRange(editor, options === null || options === void 0 ? void 0 : options.at, options);
...
const selectBlockById = (editor, id) => {
  ...
  if (!path) return;
  plateCore.select(editor, plateCore.getRange(editor, path));
  plateCore.focusEditor(editor);
};
...
const selectBlocksBySelectionOrId = (editor, id) => {
  ...
  if (isBlockSelected) {
    plateCore.select(editor, getNodesRange(editor, blockEntries));
    plateCore.focusEditor(editor);
  } else {
...
exports.DRAG_ITEM_BLOCK = DRAG_ITEM_BLOCK;
...
exports.onDropNode = onDropNode;
exports.onHoverNode = onHoverNode;
