...
const decorateCodeLine = editor => {
  const code_block = plateCore.getPlugin(editor, ELEMENT_CODE_BLOCK);
  const code_line = plateCore.getPlugin(editor, ELEMENT_CODE_LINE);
  return ([node, path]) => {
    ...
    const codeBlock = plateCore.getParentNode(editor, path);
    ...
    const text = plateCore.getNodeString(node);
...
const getCodeLineType = editor => plateCore.getPluginType(editor, ELEMENT_CODE_LINE);
...
const getCodeLineEntry = (editor, {
  ...
  if (at && plateCore.someNode(editor, {
    ...
    const selectionParent = plateCore.getParentNode(editor, at);
    if (!selectionParent) return;
    const [, parentPath] = selectionParent;
    const codeLine = plateCore.getAboveNode(editor, {
      at,
      match: {
        type: getCodeLineType(editor)
      }
    }) || plateCore.getParentNode(editor, parentPath);
    if (!codeLine) return;
    const [codeLineNode, codeLinePath] = codeLine;
    if (plateCore.isElement(codeLineNode) && codeLineNode.type !== getCodeLineType(editor)) return;
    const codeBlock = plateCore.getParentNode(editor, codeLinePath);
    if (!codeBlock) return;
    return {
      codeBlock,
      codeLine
...
const indentCodeLine = (editor, {
  ...
  const codeLineStart = plateCore.getStartPoint(editor, codeLinePath);
  ...
  if (!plateCore.isExpanded(editor.selection)) {
    ...
    const range = plateCore.getRange(editor, codeLineStart, cursor);
    const text = plateCore.getEditorString(editor, range);

    if (/\S/.test(text)) {
      plateCore.insertText(editor, '  ', {
        at: editor.selection
      });
      return;
    }
...
const deleteStartSpace = (editor, {
  ...
  const [, codeLinePath] = codeLine;
  const codeLineStart = plateCore.getStartPoint(editor, codeLinePath);
  const codeLineEnd = codeLineStart && plateCore.getPointAfter(editor, codeLineStart);
  const spaceRange = codeLineEnd && plateCore.getRange(editor, codeLineStart, codeLineEnd);
  const spaceText = plateCore.getEditorString(editor, spaceRange);
...
const onKeyDownCodeBlock = editor => e => {
  ...
  if (isTab || isUntab) {
    const _codeLines = plateCore.getNodeEntries(editor, {
      match: {
        type: getCodeLineType(editor)
      }
    });
    ...
    if (codeLines.length) {
      ...
      const codeBlock = plateCore.getParentNode(editor, firstLinePath);
      if (!codeBlock) return;
  ...
  if (isHotkey('mod+a', e)) {
    ...
    plateCore.select(editor, codeBlockPath);
    e.preventDefault();
    e.stopPropagation();
...
const insertFragmentCodeBlock = editor => {
  ...
  const codeBlockType = plateCore.getPluginType(editor, ELEMENT_CODE_BLOCK);
  const codeLineType = plateCore.getPluginType(editor, ELEMENT_CODE_LINE);

  function convertNodeToCodeLine(node) {
    return {
      type: codeLineType,
      children: [{
        text: plateCore.getNodeString(node)
      }]
    };
  }
  ...
  return fragment => {
    const inCodeLine = plateCore.findNode(editor, {
      match: {
        type: codeLineType
      }
    });
    ...
    return plateCore.insertFragment(editor, fragment.flatMap(node => {
      const element = node;
...
const getIndentDepth = (editor, {
  ...
  const [, codeLinePath] = codeLine;
  const text = plateCore.getEditorString(editor, codeLinePath);
...
const insertCodeBlock = (editor, insertNodesOptions = {}) => {
  if (!editor.selection || plateCore.isExpanded(editor.selection)) return;

  const matchCodeElements = node => node.type === plateCore.getPluginType(editor, ELEMENT_CODE_BLOCK) || node.type === getCodeLineType(editor);
  ...
  plateCore.wrapNodes(editor, {
    type: plateCore.getPluginType(editor, ELEMENT_CODE_BLOCK),
    children: []
  }, insertNodesOptions);
};
...
const insertEmptyCodeBlock = (editor, {
  ...
  if (plateCore.isExpanded(editor.selection) || !plateCore.isBlockAboveEmpty(editor)) {
    const selectionPath = plateCore.getPath(editor, editor.selection);
...
const toggleCodeBlock = editor => {
  if (!editor.selection) return;
  const codeBlockType = plateCore.getPluginType(editor, ELEMENT_CODE_BLOCK);
  const isActive = plateCore.someNode(editor, {
    match: {
      type: codeBlockType
    }
  });
  ...
  if (!isActive) {
    ...
    plateCore.wrapNodes(editor, codeBlock);
    ...
    for (const [, path] of nodes) {
      plateCore.setElements(editor, codeLine, {
        at: path
      });
    }
  }
...
const createCodeBlockPlugin = plateCore.createPluginFactory({
