...
const findMentionInput = (editor, options) => plateCore.findNode(editor, { ...options,
  match: {
    type: plateCore.getPluginType(editor, ELEMENT_MENTION_INPUT)
  }
});

const isNodeMentionInput = (editor, node) => {
  return node.type === plateCore.getPluginType(editor, ELEMENT_MENTION_INPUT);
};

const isSelectionInMentionInput = editor => findMentionInput(editor) !== undefined;

const removeMentionInput = (editor, path) => plateCore.withoutNormalizing(editor, () => {
  const node = plateCore.getNode(editor, path);
  if (!node) return;
  const {
    trigger
  } = node;
  plateCore.insertText(editor, trigger, {
    at: {
      path: [...path, 0],
      offset: 0
    }
...
const withMention = (editor, {
  ...
  const {
    type
  } = plateCore.getPlugin(editor, ELEMENT_MENTION_INPUT);
  const {
    apply,
    insertBreak,
  ...
  editor.insertFragment = fragment => {
    ...
    return plateCore.insertText(editor, fragment.map(node => stripNewLineAndTrim(plateCore.getNodeString(node))).join(''));
  };
  ...
  editor.deleteBackward = unit => {
    ...
    if (currentMentionInput && plateCore.getNodeString(currentMentionInput[0]) === '') {
  ...
  editor.insertText = text => {
    ...
    const previousChar = plateCore.getEditorString(editor, plateCore.getRange(editor, editor.selection, plateCore.getPointBefore(editor, editor.selection)));
    const nextChar = plateCore.getEditorString(editor, plateCore.getRange(editor, editor.selection, plateCore.getPointAfter(editor, editor.selection)));
    ...
    if ((beginningOfLine || precededByWhitespace) && (endOfLine || followedByWhitespace)) {
      ...
      return plateCore.insertNodes(editor, data);
    }
  ...
  editor.apply = operation => {
    ...
    if (operation.type === 'insert_text' || operation.type === 'remove_text') {
      ...
      if (currentMentionInput) {
        plateCombobox.comboboxActions.text(plateCore.getNodeString(currentMentionInput[0]));
      }
...
const createMentionPlugin = plateCore.createPluginFactory({
  key: ELEMENT_MENTION,
  isElement: true,
  isInline: true,
  isVoid: true,
...
const getMentionOnSelectItem = ({
  ...
  } = plateCore.getPlugin(editor, key);
  const pathAbove = (_getBlockAbove = plateCore.getBlockAbove(editor)) === null || _getBlockAbove === void 0 ? void 0 : _getBlockAbove[1];

  const isBlockEnd = () => editor.selection && pathAbove && plateCore.isEndPoint(editor, editor.selection.anchor, pathAbove);
  ...
  plateCore.withoutNormalizing(editor, () => {
    ...
    plateCore.select(editor, targetRange);
    plateCore.withoutMergingHistory(editor, () => plateCore.removeNodes(editor, {
